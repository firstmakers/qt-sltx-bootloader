...
void Comm::PollUSB()
{
    ...
    hid = hid_enumerate(HID_VID, HID_PID);
    if((hid != NULL)||(dev != NULL))
        connected = true;
    else
        connected = false;

    boot_status = (dev != NULL);
    hid_status =(hid != NULL);
    qWarning("Device connected :  %s", connected?"true":"false");

    if(hid != NULL){
        hid_free_enumeration(hid);
        qWarning("Slt1 hid mode");
    }if(dev != NULL){
        hid_free_enumeration(dev);
        qWarning("Slt1 bootloader mode");
    }
}
 ...
 *
 */
bool Comm::isHidMode(void)
{
    return hid_status;
}

bool Comm::isBootloaderMode(void){
    return boot_status;
}
/**
 *
...
Comm::ErrorCode Comm::openHID(void)
{
    boot_device = hid_open(HID_VID, HID_PID, NULL);
    if(boot_device)
    {
        connected = true;
        hid_set_nonblocking(boot_device, true);
...
void Comm::Reset(void)
{
    ...
    if(connected) {
        ...
        if(status == Comm::Success)
            qDebug("Successfully sent reset command");
        else
            qWarning("Sending reset command failed.");
    }
}

void Comm::TurnOnBootLoaderMode(void){
    unsigned char sendPacket[65];
    QTime elapsed;
    ErrorCode status;
    ...
    if(connected) {
        ...
        if(status == Comm::Success){
            qDebug("Modo bootloader activado", (double)elapsed.elapsed() / 1000);
            connected = false;
        }
        else
            qWarning("Error en el comando bootloader");
    }
}
