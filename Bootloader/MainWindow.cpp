...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindowClass)
{
   ...
   // setWindowTitle(APPLICATION + QString(" v") + VERSION);
    ...
    comm->PollUSB();
    if(comm->isConnected() && comm->isHidMode()){
        comm->openHID();
        comm->TurnOnBootLoaderMode();
        comm->close();
        comm->PollUSB();
    }
    if(comm->isConnected() && comm->isBootloaderMode())
    {
...
void MainWindow::Connection(void)
{
    ...
    if(currStatus != comm->isConnected())
    {
        ...
        if(comm->isHidMode()){
            qWarning("Reiniciando en modo Bootloader");
            comm->openHID();
            comm->TurnOnBootLoaderMode();
            comm->close();
            comm->PollUSB();
           // qWarning(" conectando... " + comm->boot_device);
        }
